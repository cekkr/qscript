...
class PsiScriptParser:
    ...
    def _parse_operation(self, stmt: str) -> Optional[PsiOperation]:
        if ".Superpose" in stmt:
            reg, args = self._split_call(stmt, ".Superpose")
            params = self._parse_key_values(args)
            targets = self._parse_targets(params.get("targets", "ALL"))
            return PsiOperation(kind="superpose", register=reg, targets=targets, raw=stmt)

        if ".Phase" in stmt:
            reg, args = self._split_call(stmt, ".Phase")
            params = self._parse_key_values(args)
            angle = self._eval_angle(params.get("angle", "0"))
        ...
        if ".Flip" in stmt:
            reg, args = self._split_call(stmt, ".Flip")
            params = self._parse_key_values(args)
        ...
        if ".Reflect" in stmt:
            reg, args = self._split_call(stmt, ".Reflect")
            params = self._parse_key_values(args)
    ...
    def _split_call(self, stmt: str, token: str) -> Tuple[str, str]:
        reg = stmt.split(token, 1)[0].strip()
        start = stmt.index("(", stmt.index(token))
        end = stmt.rindex(")")
        return reg, stmt[start + 1 : end]

    def _parse_key_values(self, args: str) -> Dict[str, str]:
        items: Dict[str, str] = {}
    ...
    def _parse_targets(self, value: str) -> List[int]:
        value = value.strip()
        if value.upper() == "ALL":
            return ["ALL"]  # sentinel
    ...
    def _eval_angle(self, text: str) -> float:
...
class QuantumSimulator:
    ...
    def _apply_phase(self, op: PsiOperation):
        predicate = self._build_quantum_predicate(op.predicate) if op.predicate else None
        when_fn = self._build_classical_predicate(op.when) if op.when else None
    ...
    def _build_quantum_predicate(self, expr: str) -> Callable[[List[int]], bool]:
    ...
    def _build_classical_predicate(self, expr: str) -> Callable[[Dict[str, int]], bool]:
        sanitized = expr.replace("&&", " and ").replace("||", " or ").replace("!", " not ")
